...
class Occupied_Events:
    @classmethod
    def find(self, id = None, guide_oc_id = None, room_oc_id = None, available_event_id = None) -> [()]:
        try:
            placeholder = ""
            values = ()
            if id != None:
    ...
    @classmethod
    def new(self, guide_oc_id, room_oc_id, busy_from, busy_to, available_event_id, comment) -> int:
        try:
            if len(self.find(guide_oc_id=guide_oc_id, room_oc_id=room_oc_id)) > 0:
                log(f"Duplicate occupied event already exists")
                return 0
    ...
    def delete(self, id) -> int:
        try:
            if len(self.find(id=id)):
                log(f"No occupied event [{id}] exists to delete")
                return 0
    ...
    def change_time(self, id, new_from, new_to) -> id:
        try:
            if len(self.find(id=id)) < 1:
                log(f"No occupied event found for [{id}] time change")
                return -1
    ...
    def change_comment(self, id, comment):
        try:
            if len(self.find(id=id)) < 1:
                log(f"No occupied event found for [{id}] comment change")
                return -1

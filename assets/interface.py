...
class Occupied_Events:
    ...
    @classmethod
    def new(self, busy_from, busy_to, available_event_id, comment) -> int:
        
        try:
            if available_event_id < 1:
                raise ValueError(config.errors.id_below_one)
            if len(self.find(available_event_id=available_event_id, busy_from=busy_from, busy_to=busy_to)) > 0:
                raise FindError(Errors.duplicate_found)
            ...
            room_oc_id = Rooms.occupie(event[0][2], busy_from, busy_to, f"Event {Events.get_name(event[0][1])} with guide {[Guides.get_name(event[0][3])]}")
            if room_oc_id < 1:
                raise FailedMethodError(Errors.occupie_failed)
